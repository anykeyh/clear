...
module Clear::SQL::Logger
  SQL_KEYWORDS = Set(String).new(%w(
    ...
    GROUP HAVING IN INSERT INITIALLY INTERSECT INTO LEADING LIMIT
    LOCALTIME LOCALTIMESTAMP NEW NOT NULL OFF OFFSET OLD ON ONLY OR
    ORDER PLACING PRIMARY REFERENCES RETURNING SELECT SESSION_USER SET SOME
    SYMMETRIC TABLE THEN TO TRAILING TRUE UNION UNIQUE UPDATE USER USING
    VALUES WHEN WHERE
  ))
